...
void FileListModel::refreshPreviews()
{
    qDebug() << "FileListModel::refreshPreviews - 当前图标大小:" << m_iconSize;
    QVector<int> roles = {PreviewPathRole, Qt::SizeHintRole};
    emit dataChanged(index(0), index(rowCount() - 1), roles);
    emit layoutChanged();
}

void FileListModel::setIconSize(int size)
{
    qDebug() << "FileListModel::setIconSize:" << size;
    if (m_iconSize != size) {
        m_iconSize = size;
...
void FileListModel::setPreviewQuality(const QString &quality)
{
    if (m_previewQuality != quality) {
        m_previewQuality = quality;
